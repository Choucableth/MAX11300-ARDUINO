...
bool MAX11300::begin(void) {
  ...
  uint16_t result = readRegister(MAX_DEVICE_ID);
...
pinMode_t MAX11300::getPinMode(uint8_t pin) {
  ...
  if (conf == MAX_FUNCID_DAC_ADCMON) return analogOutMonitoring;
...
bool MAX11300::isAnalogDataReady(uint8_t pin) {
  if (readRegister(MAX_INT) & MAX_ADCDR_MASK) {
    ...
    _analogStatus |= (((uint32_t)(readRegister(MAX_ADCST_H))) << 16) | (readRegister(MAX_ADCST_L));
  }
...
bool MAX11300::MAX11300regWrite(int regAddress8, int regData16) {
  return writeRegister(regAddress8, regData16);
}

void MAX11300::MAX11300initDelayus(int delay_us) {
...
bool MAX11300::writeRegister(uint8_t address, uint16_t* values, uint8_t size) {
  ...
  _spi->transfer((address << 1) | 0x00);
  uint8_t* ptrValue = (uint8_t*)values;
  for (uint8_t i = 0; i < size; i++) {
#ifdef DEBUG
    Serial.println(*(ptrValue + 1 + i), HEX);
    Serial.println(*(ptrValue + i), HEX);
#endif
    _spi->transfer(*(ptrValue + 1 + i));
    _spi->transfer(*(ptrValue + i));
  }
  digitalWrite(_select, HIGH);
  _spi->endTransaction();

#ifdef DEBUG
  uint16_t test = readRegister(address);
  Serial.print("Valeur relu : ");
